...
const API_URL = process.env.NEXT_PUBLIC_CATALOG_API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'
...
export default function AdminFloatingButton({ slug }: { slug: string }) {
  ...
  async function callAction(action: 'activate' | 'archive') {
    ...
    try {
      ...
      await fetch(`${API_URL}/api/v1/admin/businesses/${slug}/${action}`, {
        method: 'POST',
