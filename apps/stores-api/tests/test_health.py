...
def mock_firestore():
    ...
    with (
        ...
        patch("app.application.use_cases.prospects.get_db", return_value=db),
...
def test_storefront_known_slug(client):
    response = client.get("/api/v1/stores/heladeria-pinguino")
    assert response.status_code == 200
    data = response.json()
