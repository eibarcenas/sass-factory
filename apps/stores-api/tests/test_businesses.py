...
Patch the local import reference (app.application.use_cases.businesses.get_db),
...
def test_list_businesses_returns_all(client):
    ...
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = client.get("/api/v1/platform/businesses")
...
def test_list_businesses_requires_auth():
    with patch.dict(os.environ, {"DEV_USER_EMAIL": ""}):
        ...
        db = _db_with([])
        with patch("app.application.use_cases.businesses.get_db", return_value=db):
            resp = c.get("/api/v1/platform/businesses")
...
def test_valid_status_transition(client, from_status, action):
    ...
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = client.post(f"/api/v1/platform/businesses/biz-1/actions/{action}")
    assert resp.status_code == 200
...
def test_unknown_business_returns_404(client):
    db = _db_with([])
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = client.post("/api/v1/platform/businesses/nonexistent/activate")
