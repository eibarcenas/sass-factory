...
Tests three auth scenarios for seller product endpoints:
...
def _db_with_business(slug: str):
    ...
    col = MagicMock()
    ref = MagicMock()
    ref.get.return_value = MagicMock(exists=True, id=slug, to_dict=lambda: {"name": "Test", "slug": slug, "status": "store"})
...
# ---------------------------------------------------------------------------
# SUPER_ADMIN — GET /seller/products
# ---------------------------------------------------------------------------

def test_super_admin_with_business_slug_returns_200(super_admin_client):
    db = _db_with_business("heladeria-el-pinguino")
    with (
        patch("app.application.use_cases.businesses.get_db", return_value=db),
        patch("app.application.use_cases.items.get_db", return_value=db),
    ):
        resp = super_admin_client.get("/api/v1/seller/products?business=heladeria-el-pinguino")
...
def test_super_admin_without_business_slug_returns_400(super_admin_client):
    ...
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = super_admin_client.get("/api/v1/seller/products")
    assert resp.status_code == 400
    assert "SUPER_ADMIN must provide" in resp.json()["detail"]


# ---------------------------------------------------------------------------
# OWNER — GET /seller/products
# ---------------------------------------------------------------------------
...
def test_owner_reads_from_jwt_claims_returns_200(owner_client):
    ...
    with (
        ...
        resp = owner_client.get("/api/v1/seller/products")
    assert resp.status_code == 200
    assert "items" in resp.json()
...
def test_owner_ignores_business_query_param(owner_client):
    ...
    with (
        ...
        resp = owner_client.get("/api/v1/seller/products?business=other-business")
    assert resp.status_code == 200


# ---------------------------------------------------------------------------
# SUPER_ADMIN — PATCH /seller/profile
# ---------------------------------------------------------------------------
...
def test_super_admin_patch_business_with_slug_returns_200(super_admin_client):
    ...
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = super_admin_client.patch(
            "/api/v1/seller/profile?business=heladeria-el-pinguino",
...
def test_super_admin_patch_business_without_slug_returns_400(super_admin_client):
    db = _db_with_business("heladeria-el-pinguino")
    with patch("app.application.use_cases.businesses.get_db", return_value=db):
        resp = super_admin_client.patch("/api/v1/seller/profile", json={"tagline": "test"})
    assert resp.status_code == 400
