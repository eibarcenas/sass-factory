    ...
    <div class="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div
          ...
          :style="{ background: business.theme.primary }"
        >
          {{ business.theme.emoji }}
        </div>
        <div>
          <h1 class="font-bold text-gray-900 text-sm leading-tight">{{ business.name }}</h1>
          <p class="text-xs text-gray-500">{{ business.city }}</p>
        </div>
      </div>
      <a
        ...
        class="flex items-center gap-1.5 px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-medium rounded-full transition-colors"
      >
