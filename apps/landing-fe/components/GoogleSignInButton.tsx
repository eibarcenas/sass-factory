...
export default function GoogleSignInButton({
  ...
  const paddingClass = size === 'large' ? 'px-7 py-3.5 text-[1rem]' : 'px-5 py-3 text-sm'
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      <button
        onClick={handleClick}
        disabled={loading}
        className={`flex items-center gap-3 font-semibold rounded-lg transition-all duration-150
          hover:bg-gray-100 active:scale-[0.98] disabled:opacity-60 bg-white text-[#111]
          ${paddingClass}`}
      >
        {!loading && (
