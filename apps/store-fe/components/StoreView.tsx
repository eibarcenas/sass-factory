...
function StoreAcceptanceModal({ slug, onClose }: { slug: string; onClose: () => void }) {
  ...
  async function submit() {
    ...
    setError('')
    try {
      const res = await fetch(`/api/stores/${slug}/acceptance`, {
        method: 'POST',
...
function getShowPrices(data: StoreData) {
  const config = data as StoreData & { show_prices?: boolean; showPrices?: boolean }
  return config.show_prices ?? config.showPrices ?? true
}

export default function StoreView({ data, isReview = false }: { data: StoreData; isReview?: boolean }) {
  const [activeCategory, setActiveCategory] = useState('')
  const [showStoreAcceptance, setShowStoreAcceptance] = useState(false)
  ...
  function sendCartRequest() {
    ...
    fetch(`/api/stores/${data.slug}/requests`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        hash,
    ...
    <div>
      {isReview && (
        <div className="sticky top-0 z-20 bg-primary text-primary-foreground px-4 py-2.5 flex items-center justify-between gap-3">
          <p className="text-sm font-medium">✨ Your store is ready to activate.</p>
          <Button size="sm" variant="secondary" className="shrink-0 rounded-full h-7 text-xs"
            onClick={() => setShowStoreAcceptance(true)}>
            Yes →
          </Button>
        </div>
      )}

      <StoreHeader business={data} cartCount={cartCount} onOpenCart={() => setShowCart(true)} />
      ...
      {showStoreAcceptance && (
        <StoreAcceptanceModal slug={data.slug} onClose={() => setShowStoreAcceptance(false)} />
      )}
