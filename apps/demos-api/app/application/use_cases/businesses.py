"""Business use cases: orchestrate domain rules and side effects."""
...
def transition_status(business_id: str, action: str) -> dict:
    ...
    if new_status == BusinessStatus.ACTIVE and owner_email:
        email_gateway.send_catalog_activated(owner_email, name, business_id)
    elif new_status in {BusinessStatus.REVIEW, BusinessStatus.PENDING_REVIEW}:
        email_gateway.send_review_submitted(name, business_id, owner_email)
